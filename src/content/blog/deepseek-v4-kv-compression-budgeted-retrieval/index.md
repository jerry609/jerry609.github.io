---
title: 'DeepSeek-V4 的 KV 压缩：不是删 token，而是预算检索'
...
> LLM 的长上下文注意力，本质上是一个带预算的检索/汇聚问题。DeepSeek-V4 不是在判断某个 token 有没有语义价值，而是在判断当前 query 值不值得花计算预算把某段记忆展开读取。

如果用一句人话概括 DeepSeek-V4 的混合注意力设计，它像是在把完整历史上下文改造成一个分层记忆系统：

> 近处细看，远处摘要，关键远处再检索。

被 top-k 排除在外的 token 不是“无意义”，而是对**当前层、当前 query、当前 head/分支**来说，没有被单独展开读取。它们可能已经被压进 block 摘要里，可能仍在 HCA 的全局压缩记忆里，也可能只是在这个 query 上边际贡献很小。
...
但 DeepSeek-V4 的 CSA，即 Compressed Sparse Attention，不是随机丢 token。它先把一小段历史压成一个可检索的 memory entry：

$$
z_b=\Phi(h_{bm},\ldots,h_{bm+m-1})
$$

然后当前 query 不再从所有原始 token 里检索，而是先从压缩后的 block memory 中检索：
...
这一步非常关键。CSA 的对象不是“原始 token 是否存在”，而是“某个压缩记忆块是否值得被当前 query 访问”。

## 2. 为什么 top-k 可以工作：attention 常常是稀疏贡献的

对某个 query 来说，完整 attention 的输出是很多 value 的加权和。真正重要的不是“上下文里有多少 token”，而是 attention mass 怎么分布。
...
> 不是问 token 有没有语义意义，而是问它在当前 attention 输出里的残余贡献 mass 有多大。

当然，DeepSeek-V4 选的是 compressed block，不是原始 token。因此实际误差还要加上压缩本身的失真：
...
第一，重要信息是否集中在少数 block 里。很多语言任务确实如此。下一 token 通常强依赖最近上下文、当前指令、若干实体、若干证据段，而不是均匀依赖全部 100 万 token。

第二，被压缩的 block 是否保留了“将来可能被问到”的特征。DeepSeek-V4 的压缩不是普通平均池化。报告中的 CSA 会生成 trainable projection、compression weights 和 positional bias，再通过 softmax 权重合成 compressed entry。它更像 learned memory summary，而不是把一段文本粗暴缩成一句话。
...
第二，它所在的 compressed block 被 top-k 选中了。模型不是逐 token 看它，但会读到该 block 的压缩表示。
...
如果 $\Delta_i$ 很小，它对这个预测就是低贡献；但这不是对该 token 的永久判决。
...
这说明 DeepSeek-V4 并不是在赌“top-k 万能”。它的设计更像多分辨率记忆：最近处用高清，远处用缩略图，需要的时候再点开关键缩略图。
...
报告里的训练路径很重要：训练从较短序列开始，逐步扩展到 16K、64K、1M；稀疏注意力不是一开始就启用，而是先用 dense attention warmup，之后在 64K 阶段引入 sparse attention，并先 warm up CSA 的 lightning indexer，再在大部分训练中使用 sparse attention。
...
所以这不是“压缩后模型仍然奇迹般工作”，而是“模型从训练时就知道自己只能这样读历史”。
...
每个 block 原来有 $m$ 个 token 的 KV，现在只能存一个 vector。目标不是保留全部信息，而是在固定 memory rate 下最小化预测损失：
...
最真实的目标不是复现 dense attention，而是生成质量。因此，即使某些 block 在 dense attention 里有质量，只要删掉后最终 token 分布基本不变，就可以不选。
...
报告中提到，CSA indexer 的 QK path 使用 FP4、index scores 从 FP32 量化到 BF16 后，top-k selector 获得 2x 加速，同时保持 99.7% 的 KV entry recall。这里的 recall 更像是在验证“低精度加速后选出来的 KV entry 是否接近原 selector”，不是证明 sparse attention 与 full attention 完全等价。
...
**假设 4：模型可以通过训练适应这种记忆格式。** V4 不是事后压缩，而是在长上下文训练中逐步引入 sparse attention，并 warm up indexer。

**假设 5：真实任务分布通常不是最坏情况。** 如果用户构造一个必须均匀读取全上下文的算法题，top-k sparse attention 可能比 full attention 更吃亏。

## 9. 一句话总结

DeepSeek-V4 的 KV 压缩不是在说：
...
top-k 不是在判断“语义上有没有价值”，而是在判断“当前 query 值不值得花计算预算把这个 memory block 展开读”。
